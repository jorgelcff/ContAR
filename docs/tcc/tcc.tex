...
\local{Recife}
\data{2026}
\orientador{Prof. Dr. Francisco Paulo Magalhaes Simões}
...
\preambulo{
  ...
  obtenção do grau de Bacharel em Sistemas de Informações.
}
  ...
  \begin{center}
    \fbox{
      \begin{minipage}[c][8cm]{13cm}
        ...
        \hspace*{0.5cm}
        Orientador: Francisco Paulo Magalhaes Simões.\\
        ...
        5.~No-Code.
        6.~Educação.
        I.~Francisco Paulo Magalhaes Simões, orient.
        II.~Título.
      \end{minipage}
    }
  \end{center}
...
\begin{agradecimentos}
  Agradeço ao meu orientador, Prof. Dr. Francisco Simões, pelo acompanhamento, pelas
  discussões que moldaram as decisões desta pesquisa e pela
...
O sistema de poses suporta 18 presets: 7 animadas (idle, walk,
walk\_circle, slow\_run, run, dance, speaker) e 11 estáticas (neutral,
wave, hands on hips, salute, arms crossed, think, point, bow, pray,
shrug, t-pose). A pose \texttt{walk\_circle} implementa uma órbita
procedural ao redor de um ponto central, útil para indicar deslocamento
do narrador sem alterar sua posição de ancoragem na cena. As poses
estáticas são implementadas como rotações procedurais de ossos no
sistema de coordenadas local, usando uma abstração que compensa a
orientação do rig exportado (\texttt{rotateBoneDeg}).
...
\subsection{Transformações por Pose e Modos de Exibição de Texto}
\label{subsec:transform-display}

O editor oferece controles de transformação independentes para
posição (X, Y, Z), rotação (X, Y, Z) e escala do avatar, ajustáveis
por sliders com retroalimentação imediata na cena 3D. Cada combinação
de valores é armazenada associada à pose ativa (\texttt{transformByPose}):
ao alternar entre presets de pose, o editor restaura automaticamente os
ajustes de transformação previamente definidos para aquela pose,
evitando que o usuário precise reposicionar o avatar a cada troca.
Um botão de reset (``Limpar'') restaura os valores padrão (posição e
rotação iguais a zero, escala igual a um) para a pose selecionada.

Para a apresentação textual da fala, o ContAR oferece três modos de
exibição configuráveis pelo autor: balão de fala sobreposto ao avatar
(\textit{bubble}, padrão), legenda na parte inferior da tela
(\textit{subtitle}, no estilo de legendas de vídeo) ou texto oculto
(\textit{none}). Essa flexibilidade permite adaptar a apresentação ao
contexto pedagógico — por exemplo, priorizando a compreensão auditiva
do narrador ou oferecendo suporte de leitura simultânea para alunos
...
orientador Prof. Dr. Francisco Paulo Magalhaes Simões, representa a fronteira de pesquisa mais
