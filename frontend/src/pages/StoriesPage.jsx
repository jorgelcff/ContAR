    ...
    <div className="group rounded-2xl border border-gray-700 bg-gray-800 p-5 flex flex-col gap-4 hover:border-gray-600 transition-colors">
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="shrink-0 flex flex-col items-end gap-1">
          <div className="flex items-center gap-1 rounded-full bg-gray-700/60 px-2.5 py-1 text-[11px] text-gray-300">
          ...
          <div className={`flex items-center gap-1 rounded-full px-2.5 py-1 text-[11px] ${
            story.isPublic ? 'bg-emerald-900/40 text-emerald-300' : 'bg-gray-700/60 text-gray-400'
          }`}>
            <Icon name={story.isPublic ? 'unlock' : 'lock'} className="w-3.5 h-3.5" />
            <span>{story.isPublic ? t('storiesCardPublished') : t('storiesCardDraft')}</span>
          </div>
        </div>
      </div>
      ...
      {story.isPublic ? (
        ...
        <p className="text-[11px] text-gray-500 italic">{t('storiesCardNotPublishedHint')}</p>
      )}
      ...
      <div className="flex items-center gap-2">
        <Link
          ...
          className="flex-1 text-center py-2 rounded-xl bg-cyan-700 hover:bg-cyan-600 text-white text-xs font-medium transition-colors"
        >
          {t('storiesCardEdit')}
        </Link>
        <Link
        ...
        <div className="max-w-4xl mx-auto px-4 md:px-6 py-6 flex flex-col gap-6">
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="px-4 py-2 rounded-xl bg-cyan-700 hover:bg-cyan-600 text-white text-sm font-semibold transition-colors"
              >
          ...
          {showCreate && (
            <form
              ...
              className="rounded-2xl border border-cyan-700/40 bg-cyan-950/20 p-5 flex flex-col gap-3"
            >
              <p className="text-sm font-semibold text-cyan-300">{t('storiesCreateSectionTitle')}</p>
              <input
                ref={titleRef}
                type="text"
                value={newTitle}
                ...
                className="w-full rounded-xl bg-gray-900 border border-gray-700 text-white text-sm px-3 py-2.5 placeholder-gray-500 focus:outline-none focus:border-cyan-500"
              />
              <textarea
                rows={2}
                value={newDesc}
                ...
                className="w-full rounded-xl bg-gray-900 border border-gray-700 text-white text-sm px-3 py-2 placeholder-gray-500 focus:outline-none focus:border-cyan-500 resize-none"
              />
              <div className="flex gap-2">
                <button
                  type="submit"
                  disabled={saving}
                  className="px-5 py-2.5 rounded-xl bg-cyan-700 hover:bg-cyan-600 disabled:opacity-50 text-white text-sm font-medium transition-colors"
                >
          ...
          {loading ? (
            ...
            <div className="rounded-2xl border border-gray-700 bg-gray-800/50 p-12 flex flex-col items-center gap-5 text-center">
              ...
              <button
                ...
                className="px-6 py-3 rounded-xl bg-cyan-700 hover:bg-cyan-600 text-white text-sm font-semibold transition-colors"
              >
                {t('storiesEmptyBtn')}
              </button>
